...
@section('content')
    <div class="flex justify-between mb-3">
        ...
        <x-toolbar createRoute="admin.products.create" trashRoute="admin.products.trash" />
    </div>
                ...
                @else
                    @foreach ($products as $product)
                        <tr>
                            ...
                            <td>{{ $product->discount_percent }}% => {{ number_format($product->final_price, 0, ',', '.')}} đ</td>
                            <td>{{ $product->views }}</td>
                            <td>
                                <x-status-badge :status="$product->status" />
                            </td>
                            <td>
                                <x-action-buttons type="index" :id="$product->id" :name="$product->name" routeName="products" />
                            </td>
                        </tr>
                    @endforeach
                @endif
